...
class ImageCheckCallback(K.callbacks.Callback):
    ...
    def make_current_save_dict(self):
        current_save_dict = self.save_dir + self.save_name + \
    ...
    def load_image(self, image_from, batch):
        image_name = self.image_names[image_from][random.randint(0, len(self.image_names[image_from]))]
        ...
        label = np.floor(np.array(Image.open(label_source)) * 3.7).astype(np.uint8)
    ...
    def on_batch_end(self, batch, logs=None):
        if batch % self.n_batch_log == 0:
            ...
            label = np.floor(train_predict * 3.7).astype(np.uint8)
            label = cv2.applyColorMap(label, cv2.COLORMAP_HSV)
            cv2.imwrite(os.path.join(self.current_save_dict, predicted_train_file_name + '.png'), label)
            label = np.floor(validation_predict * 3.7).astype(np.uint8)
            label = cv2.applyColorMap(label, cv2.COLORMAP_HSV)
